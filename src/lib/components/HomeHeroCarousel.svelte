...
<script lang="ts">
    ...
    // Animate margin when data becomes available
    $: if (hasData && carouselItems.length > 0 && marginClass === '-mb-2') {
        marginClass = '-mb-20';
    }
    ...
    afterUpdate(() => {
        if (
            ...
            currentIndex = 0;
            fadeClass = 'opacity-100';
            marginClass = '-mb-2'; // Reset margin for animation
            hasData = false; // Reset data flag
            stopCarousel();
            resetResumeTimeout();
            startCarousel();
        }
...
{#if showCarousel && carouselItems.length > 0 && currentItem}
    <section 
        class="relative w-full h-[70vh] min-h-[550px] overflow-hidden cursor-pointer group {marginClass} transition-all duration-1000 ease-out"
